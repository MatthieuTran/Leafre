...
func TestWriteCheckPasswordResultNotSuccess(t *testing.T) {
	...
	writer.WriteCheckPasswordResult(&b, writer.SendCheckLogin{Result: byte(user.LoginResponseAuthFail)})
...
func TestWriteCheckPasswordResultSuccess(t *testing.T) {
	var b bytes.Buffer
	id := uint32(rand.Intn(500))

	writer.WriteCheckPasswordResult(&b, writer.SendCheckLogin{Result: byte(user.LoginResponseSuccess), ID: id})
